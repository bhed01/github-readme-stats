...
describe('Test renderTopLanguages', () => {
	...
	it('should resize the height correctly depending on langs', () => {
		...
		expect(document.querySelector('svg')).toHaveAttribute('height', '188');
	});
	...
	it('should render default colors properly', () => {
		...
		expect(langNameStyles.fill).toBe('#000c');
		expect(queryByTestId(document.body, 'card-bg')).toHaveAttribute('fill', '#fffefe44');
	});
	...
	it('should render with layout compact', () => {
		...
		expect(queryAllByTestId(document.body, 'lang-progress')[0]).toHaveAttribute('width', '140.00');

		expect(queryAllByTestId(document.body, 'lang-name')[1]).toHaveTextContent('javascript 40.00%');
		expect(queryAllByTestId(document.body, 'lang-progress')[1]).toHaveAttribute('width', '140.00');

		expect(queryAllByTestId(document.body, 'lang-name')[2]).toHaveTextContent('css 20.00%');
		expect(queryAllByTestId(document.body, 'lang-progress')[2]).toHaveAttribute('width', '70.00');
	});
